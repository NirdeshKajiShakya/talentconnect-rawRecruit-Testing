...
import { useNavigate, useParams } from 'react-router-dom';
import { ArrowLeft, Building, MapPin, Users, Calendar, Award, Briefcase, DollarSign } from 'lucide-react';
...
function JobDetail() {
    ...
    // State to hold job data and loading status
        ...
        const fetchJobDetails = async () => {
            ...
            try {
                ...
                if (response?.data?.response) {
                    ...
                    setJob(null);
                    setError("Job not found or unexpected response format.");
                }
            } catch (err) {
                setJob(null);
                // The error object is now caught here.
    ...
    const handleAccept = () => {
        console.log(`Accepting drive for job ID: ${jobId}`);
        // Implement API call to accept drive
    };
    
    const handleShortlist = () => {
        console.log(`Shortlisting drive for job ID: ${jobId}`);
        // Implement API call to shortlist drive
    };
    
    const handleReject = () => {
        console.log(`Rejecting drive for job ID: ${jobId}`);
        // Implement API call to reject drive
    };
    ...
    const { collegePosted, companyPosted } = job;
    const details = collegePosted || companyPosted; // Use correct data source
    const companyName = details?.name || 'N/A';
    const companyLocation = details?.location || 'N/A';
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-10">
            <div className="max-w-4xl mx-auto">
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                            ...
                            <div className="flex-1">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 mt-2 text-gray-600">
                                    <a href="#" className="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">
                                        {/* Dynamically generate website link */}
                            ...
                            <div className="mt-4 md:mt-0">
                                <div className="flex gap-2">
                                    <div className="text-right">
                                        <div className="text-sm text-gray-500">Industry</div>
                                        <div className="font-medium">{details?.industry || 'N/A'}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="mb-4">
                                    <div className="text-sm text-gray-500">Job Type</div>
                                    <div className="font-medium">{job.jobType || 'N/A'}</div>
                                </div>
                                
                                <div>
                                    <div className="text-sm text-gray-500">Job Location</div>
                                    <div className="font-medium">{Array.isArray(job.location) ? job.location.join(', ') : job.location || 'N/A'}</div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="mb-4">
                                    ...
                                    <div className="font-medium">{Array.isArray(job.degree) ? job.degree.join(', ') : 'N/A'}</div>
                                </div>
                                
                                <div className="mb-4">
                                    <div className="text-sm text-gray-500">Compensation</div>
                                    <div className="font-medium">
                                        {job.minPackage?.amount ? `${job.minPackage.amount} ${job.minPackage.currency || 'LPA'}` : 'N/A'}
                                    </div>
                                </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-6">
                            {job.selectionProcess?.length > 0 ? (
                                ...
                                <p className="text-gray-500">No selection process details provided.</p>
                            )}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="flex items-center">
                                ...
                                <div>
                                    <div className="text-sm text-gray-500">Student Count</div>
                                    <div className="font-medium">{job.numberOfStudents || 'N/A'}</div>
                                </div>
                            </div>
                ...
                <div className="bg-white rounded-lg shadow-sm mb-6">
                    <div className="p-6">
                        <h2 className="text-lg font-bold mb-4">Amenities Required</h2>
                        
                        <ul className="space-y-2">
                            {job.amenitiesRequired?.length > 0 ? (
                                job.amenitiesRequired.map((facility, index) => (
                                    <li key={index} className="flex items-start">
                                        <div className="h-5 w-5 rounded-full bg-green-100 flex items-center justify-center mr-3 mt-0.5">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3 h-3 text-green-600">
                                                <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                ...
                                <li className="text-gray-500">No amenities required.</li>
                            )}
                        </ul>
                    </div>
                </div>
